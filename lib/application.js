...
Application.init = function(opts){
	...
	this.serverInfo = null			// current server info
	...
	this.serverTypeMaps = {}		// current global server type maps, type -> [info]

	appUtil.defaultConfiguration(this);

	this.state = STATE_INITED;
	console.log('[%s] Application inited: %s.', utils.format(new Date), this.getServerId());
};
...
Application.start = function(cb){
	...
	appUtil.startByType(self, function(){
		...
		var startUp = function(){
			console.log('[%s] Application startUp.', utils.format(new Date))
		}
...
Application.loadConfigBaseApp = function(key, val){
	...
	if(fs.existsSync(originPath)){
		...
		console.log('invalid configuration with file path: %s', key);
	}
};
