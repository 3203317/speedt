...
function bindEvents(self, socket){
	...
	socket.on('disconnect', function(){
		console.log('[%s] Socket is closed.', utils.format(new Date));
	});

	socket.on('error', function (err){
		console.log('[%s] Socket error: %j.', utils.format(new Date), err);

	});

	socket.on('message', function (msg){
		console.log('[%s] Socket msg: %j.', utils.format(new Date), msg);
	});
}
