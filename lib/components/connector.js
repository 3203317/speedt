...
function bindEvents(self, socket){
	...
	socket.on('disconnect', function(){
		console.log('[%s] Client Socket is closed.', utils.format(new Date));
	});

	socket.on('error', function (err){
		console.log('[%s] Client Socket error: %j.', utils.format(new Date), err);

	});

	socket.on('message', function (msg){
		console.log('[%s] Client Socket msg: %j.', utils.format(new Date), msg);
	});
}
